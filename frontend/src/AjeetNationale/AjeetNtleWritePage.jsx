...
export default function AjeetNtleWritePage() {
  ...
  const [catNational, setCatNational] = useState([]); //Liste de toutes les categories
  ...
  const handleClick = async (e) => {
    ...
    try {
      state // On demande s'il ya un "state", si oui, cela signifie que c'est une  page de mise à jour donc on utilise notre route de "update"
        ? await axios.put(
            `http://localhost:1000/api/postAjeetNtles/update/${state._id}`,
            {
              title,
              cat,
              desc: value,
        ...
        : //Dans le 2nd cas, il n'ya pas de "state" donc on crée un nouveau post
          await axios.post(
            `http://localhost:1000/api/postAjeetNtles/create`,
            {
              title,
              cat,
              desc: value,
      ...
      navigate('/postAjeetNtleBiographie'); //Si l'article est crée sans problème, on sera dirigé vers la page d'accueil
    } catch (err) {
      // console.log(err);
    }
  };
  ...
  return (
    <div className="write_div">
      <Helmet>
        <title> Histoire de l'AJEET NATIONALE</title>
      </Helmet>
      <div className="">
        {' '}
        <h1> Créer Histoire de l'AJEET NATIONALE</h1>
        <Form.Control
