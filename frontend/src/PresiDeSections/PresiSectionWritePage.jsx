...
export default function PresiSectionWritePage() {
  ...
  const handleClick = async (e) => {
    ...
    try {
      state // On demande s'il ya un "state", si oui, cela signifie que c'est une  page de mise à jour donc on utilise notre route de "update"
        ? await axios.put(
            `http://localhost:1000/api/presiSections/update/${state._id}`,
            {
              name,
              lastname,
              cat,
        ...
        : //Dans le 2nd cas, il n'ya pas de "state" donc on crée un nouveau post
          await axios.post(
            `http://localhost:1000/api/presiSections/create`,
            {
              name,
              lastname,
              cat,
      ...
      navigate('/PresiSectionList'); //Si l'article est crée sans problème, on sera dirigé vers la page d'accueil
    } catch (err) {
      // console.log(err);
    }
  };
